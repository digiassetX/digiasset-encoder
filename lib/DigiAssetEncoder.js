...
class DigiAssetEncoder {
    ...
    async addUTXOs(UTXOs) {
        this.reset();
        checkUtxo: for (let utxo of UTXOs) {
            //check if already in utxo list and skip if it is
            for (let {txid,vout} of this._utxos) if ((utxo.txid===txid)&&(utxo.vout===vout)) continue checkUtxo;

            //lookup utxo if is not in digiassetX standard format or force lookup is on
            if ((utxo.value === undefined) || (this._lookupUTXOs)) utxo=await lookup.getUTXO(utxo.txid, utxo.vout);
            if (this._inputAddressesNeeded) await helpers.includeScriptPubKey(utxo);
            this._utxos.push(utxo);

            //set DigiByte change address to first address of first utxo without assets if we know it
            if ((this._changeDigiByte===undefined)&&(utxo.assets===undefined)&&(utxo.scriptPubKey!==undefined)&&(utxo.scriptPubKey.addresses!==undefined)) {
                this._changeDigiByte=utxo.scriptPubKey.addresses[0];
            }

            //add balance
            this._balances.DigiByte+=BigInt(utxo.value);
            if (utxo.assets!==undefined) {
                for (let {assetId,amount,cid} of utxo.assets) {
