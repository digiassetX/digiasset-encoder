    ...
    async build() {
        ...
        if (this._rules!==false) {
            ...
            for (let rule of usedRules) {
                if (!allowedRules.includes(rule)) throw new ExpectedError("Invalid Rule Detected");
            }
            ...
            if (this._rules.signers!==undefined) {
                ...
                for (let address in this._rules.signers.list) {
                    ...
                    if (listLeft[address] <= 0) throw new ExpectedError("Invalid Rule Detected: Invalid weight");
                    found += listLeft[address];
                }
                if (found < required) throw new ExpectedError("Invalid Rule Detected: Not enough signers to reach required weight");
            ...
            if (this._rules.royalties!==undefined) {
                ...
                for (/** @type{int}*/let index in txOutputs) {
                    let address = Object.keys(txOutputs[index])[0];
                    if (this._rules.royalties[address] !== undefined) throw new ExpectedError("Invalid Rule Detected: royalty addresses can not be sent assets or used as signer addresses");
                }
                ...
                if (this._rules.currency!==undefined) {
                    if (typeof this._rules.currency==="string") {
                        ...
                        if (i===undefined) throw new ExpectedError("Invalid Rule Detected: invalid exchange rate option");
                        ...
                        ) throw new ExpectedError("Invalid Rule Detected: invalid exchange rate option");
            ...
            if (this._rules.kyc!==undefined) {
                ...
                if ((this._rules.kyc.allow!==undefined)&&(this._rules.kyc.ban!==undefined)) throw new ExpectedError("Invalid Rule Detected: can't use both allow and ban");
                ...
                try {
                    for (let country of list) {
                        if (country.length!==3) throw new ExpectedError("Invalid Rule Detected: invalid country code");
                        rulesBinary.append3B40(country.toLowerCase());
                    }
                } catch (_) {
                    throw new ExpectedError("Invalid Rule Detected: invalid country code");
                }
            ...
            if ((this._rules.vote!==undefined)||(this._rules.expires!==undefined)) {
                /*
                    ...
                    1-7 byte fixed precision cutoff.
                            0 means no cutoff.
                            >= means time in ms since January 1st, 1970 values under are block count
                ...
                if (this._rules.rewritable===true) throw new ExpectedError("Invalid Rule Detected: Votes can not be part of rewritable rule asset");
                if ((this._rules.vote!==undefined)&&(this._rules.expires!==undefined)) throw new ExpectedError("Invalid Rule Detected: can't use both vote and expires");
                ...
                let cutoff=BigInt(this._rules.vote.cutoff||0n);
                if (cutoff!==0n) {
                        if (cutoff>=1577836800000n) {
                            let time=this.blockTime;
                            if (BigInt(time)>=cutoff/1000n) throw new ExpectedError("Invalid Rule Detected: Already Expired");
                        } else {
                            let height=await lookup.getHeight(this._height);
                            if (BigInt(height) > cutoff) throw new ExpectedError("Invalid Rule Detected: Already Expired");
                        }
                }

                //quick check on vote length
                let voteCount=Object.keys(this._rules.vote.options).length;
                if (voteCount>127) throw new ExpectedError("Invalid Rule Detected: To many vote options");
                ...
                rulesBinary.appendFixedPrecision(this._rules.vote.cutoff || 0n);       //cutoff

                if ((this._rules.vote.options.length===0)||(typeof this._rules.vote.options[0]=="string")) {
                    if (voteCount>constants.voteAddresses.length) throw new ExpectedError("Invalid Rule Detected: To many vote options");

                    //convert format to standard for purpose of ipfs data
                    for (let index in this._rules.vote.options) {
                        if (typeof this._rules.vote.options[index]!="string") throw new ExpectedError("Invalid Rule Detected: Can't mix vote option types");
                    ...
                    for (/** @type{int}*/let index in txOutputs) {
                        let address = Object.keys(txOutputs[index])[0];
                        if (this._rules.vote.options[address] !== undefined) throw new ExpectedError("Invalid Rule Detected: royalty addresses can not be sent assets or used as signer addresses");
                    }
                    ...
                    for (let index in this._rules.vote.options) {                          //encode weight in output balance
                        if (typeof this._rules.vote.options[index]=="string") throw new ExpectedError("Invalid Rule Detected: Can't mix vote option types");
                        if (typeof this._rules.vote.options[index].label!=="string") throw new ExpectedError("Invalid Rule Detected: Vote option label must be a string");
            ...
            if (this._rules.deflate!==undefined) {
                 ...
                 */
                if (this._aggregation!==0) throw new ExpectedError("Invalid Rule Detected: Deflationary assets must be aggregable");
                if (BigInt(this._rules.deflate)<=0n) throw new ExpectedError("Invalid Rule Detected: Deflation amount must be positive number");
            ...
            //finish rules
            if (rulesBinary.length===0) throw new ExpectedError("Invalid Rule Detected: set to false if not using rules");
        ...
        if (this._aggregation===0) {
            ...
            if (assetCounts.length>32) throw new ExpectedError("Asset can not be encoded to many outputs");
            for (let i in assetCounts) {
                data.appendInt(0,3);                            //skip=false,range=false,percent=false
                // noinspection JSCheckFunctionSignatures
                data.appendInt(parseInt(i),5);     //current output
        ...
        if (data.length/8>constants.maxOpReturnBytes) throw new ExpectedError("Asset can not be encoded to many outputs");
