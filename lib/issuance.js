 ...
 *      by default sends 100sat/byte to digiassetX to pin.  set to {} to self host
...
module.exports=async(utxos,outputs,metadata,options)=> {
    ...
    const nodes=(options.nodes===undefined)?{dgb1qjnzadu643tsfzjqjydnh06s9lgzp3m4sg3j68x:10000}:options.nodes;
    ...
    /** @type {metaData} */let ipfsData = {data: metadata};
    ...
    data.appendHex(ipfs.cidToHash(cid));                        //sha256 of ipfs data
    data.appendFixedPrecision(amountToCreate);      //amount to create
    data.appendBuffer(rulesBinary.toBuffer());      //appends the rules if any
    ...
    if (aggregation===0) {
        ...
        for (let i in assetCounts) {
            data.appendInt(0,3);                            //skip=false,range=false,percent=false
            // noinspection JSCheckFunctionSignatures,JSUnfilteredForInLoop
    ...
    for (let address in nodes) {
        // noinspection JSUnfilteredForInLoop
        let amountToPay=BigInt(nodes[address]*ipfsSize)+10n;    //add 10 sat just in case of rounding errors
