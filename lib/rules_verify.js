...
module.exports.verifyKYC=async(rule,addresses)=>{
    ...
    for (let address of addresses) {
        ...
        if (country===undefined) throw new ExpectedError(address +" is not KYC verified");
        if (revoked!==undefined) throw new ExpectedError("KYC on "+address+" has been revoked");
        if (ban) {
            //ban so country should not be in list
            if (list.indexOf(country)!==-1) throw new ExpectedError(address+" belongs to banned country");
        } else {
            if (list.indexOf(country)===-1) throw new ExpectedError(address+" does not belong to allowed country");
        }
    }
}
...
module.exports.verifyVote=async(rule,addresses)=>{
    ...
    for (let address of addresses) {
        if (allowed[address]===undefined) throw new ExpectedError(address+" is not a valid vote option");
    }
}
...
module.exports.verifySigners=async(rule,addresses)=>{
    ...
    if (found<required) throw new ExpectedError(required+" signer weight was needed. "+found+" was found.");
}
    ...
    for (let address in rule.royalties) {
        ...
        for (let output of outputs) {
            ...
            if (foundAmount>=amount) {
                ...
                if (!addShort) throw new ExpectedError("A royalty of "+amount+" must be sent to "+address);

                //check if output amount is 600sat if so throw an error because can't send assets to the royalty resipients address
                if (foundAmount===600n) throw new ExpectedError("Can not send assets to the royalty address");
